...
def process_instructions(stack, input_stack, output_stack, pointer = 0):
	stack = stack[:]
	while pointer < len(stack):
		...
		if opcode == 1:
			...
			op_input(arguments, stack, input_stack)
		elif opcode == 4:
			op_output(arguments, stack, modes, output_stack)
		...
		# pointer = (len(arguments)+1, next_pointer)[next_pointer != None]
...
def op_input(arguments, stack, input_stack):
	stack[arguments[0]] = input_stack()

def op_output(arguments, stack, modes, output_stack):
	output_stack(
		resolve_argument(arguments[0], stack, resolve_mode(0, modes))
	)
